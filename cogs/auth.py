...
async def require_auth(interaction: discord.Interaction) -> bool:
    ...
    if user_manager.is_registered(user_id) and credential_manager.has_credentials(user_id):
        return True
...
class Auth(commands.Cog):
    ...
    @app_commands.command(name="disconnect", description="Disconnect your Google account")
    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def disconnect(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            "Disconnected your Google account and cleaned up all data.",
            ephemeral=True,
        )
